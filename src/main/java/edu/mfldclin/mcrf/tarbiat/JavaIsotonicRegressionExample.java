...
public class JavaIsotonicRegressionExample {
  public static void main(String[] args) {
    ...
    JavaRDD<LabeledPoint> data = MLUtils.loadLibSVMFile(
      jsc.sc(), Resource.getPath("data/mllib/sample_isotonic_regression_libsvm_data.txt")).toJavaRDD();
    ...
    // Save and load model
    model.save(jsc.sc(), "target/tmp/" + runId + "/myIsotonicRegressionModel");
    IsotonicRegressionModel sameModel =
      IsotonicRegressionModel.load(jsc.sc(), "target/tmp/" + runId + "/myIsotonicRegressionModel");
    // $example off$
