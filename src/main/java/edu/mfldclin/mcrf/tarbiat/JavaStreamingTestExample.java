...
/**
 * Perform streaming testing using Welch's 2-sample t-test on a stream of data,
 * where the data stream arrives as text files in a directory. Stops when the
 * two groups are statistically significant (p-value < 0.05) or after a
 * user-specified timeout in number of batches is exceeded.
 *
 * The rows of the text files must be in the form `Boolean, Double`. For
 * example: false, -3.92 true, 99.32
 *
 * Usage: JavaStreamingTestExample <dataDir> <batchDuration>
 * <numBatchesTimeout>
 *
 * To run on your local machine using the directory `dataDir` with 5 seconds
 * between each batch and a timeout after 100 insignificant batches, call: $
 * bin/run-example mllib.JavaStreamingTestExample dataDir 5 100
 *
 * As you add text files to `dataDir` the significance test wil continually
 * update every `batchDuration` seconds until the test becomes significant
 * (p-value < 0.05) or the number of batches processed exceeds
 * `numBatchesTimeout`.
 */
public class JavaStreamingTestExample {
    ...
    public static void main(String[] args) throws Exception {
        // ahmad jan inam parameteri (CLI) bude man taghiresh midam
        /*
    if (args.length != 3) {
      System.err.println("Usage: JavaStreamingTestExample " +
        ...
        String dataDir = "."; // ahmad jan ino ba masiri ke file data tush hast avaz kon, masalan c:\data\
        Duration batchDuration = Seconds.apply(20l);// ahmad jan in duration time hast, nemidunam che adadi monasebe ama fek konam bayad adade bozorgi bashe
        int numBatchesTimeout = 10; // inam bayad timeout value bashe, nemidunam che adadi monasebe
